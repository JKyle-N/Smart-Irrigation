...
## CHANGED: battery read from calibrated opto ADC; INA226 removed; safety scaffolded-off
- **Battery V/I/P now come from the ADC**, not the INA226: **GPIO35 = voltage, GPIO34 = current**, through an optocoupler (nonlinear). `powerTick` reads a 64-sample trimmed mean and applies a per-channel **least-squares polynomial** `value = a0 + a1·u + a2·u² + a3·u³`, `u = raw/4095` (`readAdcTrimmed`/`applyPoly`). Calibrate with the bench tool `Test Code/ESP1_ADC_OPTO_CAL`, then **paste** its printed `a0..a3` into `calBattV`/`calBattI`, or populate NVS `adccal` (bench-tool `CalRec` layout) which `loadBattCal()` reads to override the defaults. INA226 code (include, object, init, `inaOk`) is removed; `DEV_INA` stays only as a harmless boot probe. Current is **magnitude only** → all energy counts as consumption (no charge/discharge split).
- **Battery safety = READY, NOT IMPLEMENTED:** the removed INA226-tied protections (`BATTERY_CRITICAL` emergency-stop, `controlTick` run-block, low-battery fertigation disable + `BATTERY_LOW/CRITICAL` alerts, pump-exercise gate) are present as **flag-guarded code behind `#define BATTERY_SAFETY_ENABLED 0`** (in `powerTick`/`batterySafetyTick`, `controlTick`, `exerciseTick`). Set the one define to `1` to re-enable all of it (compiles both ways). `BATT_LOW_V`/`BATT_CRIT_V` thresholds are kept; `batteryLow`/`batteryCritical` remain display/log flags. (Trade-off while off: no brown-out / deep-discharge protection.)

## ADDED: ESP32 #1 self-recovery (device-loss daily reset)
- If a core device that was **present at boot** (RTC 0x68, LCD 0x27, or SD — **not** INA226, see above) **drops out at runtime** (suspected I2C wedge), ESP1 reboots itself via `ESP.restart()` — **at most once per calendar day**.
...
- **ESP2 minimum on-time (10 s) + idle heartbeat poll** (`esp2SetPower`/`esp2PowerTick`): every ESP2 power-up now stays on **≥10 s** (a graceful `esp2SetPower(false)` while on <10 s is **deferred** via `esp2OffPending`/`esp2OffAt`; only `enterEmergencyStop` passes `force=true` to cut immediately) — a too-brief power-up can no longer leave ESP2 unable to boot/ACK/heartbeat. While idle, if ESP2 hasn't been heard for **10 min (day) / 1 h (night)** (`isNight()` reuses the Nano day/night boundary), ESP1 powers it up briefly for a health heartbeat: `IDLE_POLL|ALIVE` then graceful off, or `IDLE_POLL|NO_REPLY` → the **recovery ladder** (`RECOVERY_STATE`). Poll clock reuses `lastEsp2Ms`, so a recent run defers the next poll and back-to-back runs reuse the still-powered ESP2.

## ADDED: ESP2 comm-recovery workaround (link/UART persistent failure)
When power-cycling ESP2 (even 5×) doesn't restore comm, the fault is on **ESP1's side** (a wedged UART1 RX peripheral) or the **physical link** — resetting ESP2 alone can't fix either. Three changes in the recovery ladder (`sec.18.9`):
- **Every recovery attempt now re-inits ESP1's own UART** — `esp2ReinitUart()` (`esp2Serial.end()`/`begin()` + drain RX, logs `ESP2|UART_REINIT`) runs at the top of `esp2PowerCycle()` and before each `RESET_SELF`. This clears a framing-error/FIFO lockup on the ESP1 RX that cycling ESP2 never touched.
- **Fast-cycle cap + slow-retry** (`esp2Escalate`/`esp2CommRetryTick`, `esp2FastCycles`/`ESP2_MAX_FAST_CYCLES=5`): after 5 fast power-cycles in an episode ESP1 stops hammering the relay — latches **`ESP2_COMM_LOST`** (one-shot `raiseFault('M',…,"LINK")` SMS), leaves the fast loop (`RECOVERY_STATE` bails while latched; idle-poll gated off), and falls back to **one gentle re-init+cycle every 20 min** (`ESP2_SLOW_RETRY_MS`). Any `READY`/`STARTUP_SYNC` validate calls `esp2CommRecovered()` → clears the latch + counter + `ESP2_COMM_OK` SMS.
- **Final escalation: one ESP1 self-reset/day** — hitting the cap also calls `esp1SelfResetOncePerDay("ESP2_COMM_LOST")` (shares the RTC-RAM `g_lastSelfResetDay` guard with the device-loss reset → ≤1 self-reset/day, no boot loop) to fully reinit ESP1's peripherals.
- **Hardware caveat (no firmware can fix this):** if the link is physically broken — TX/RX crossed, **no common ground**, loose wire, baud mismatch — verify ESP1 TX25→ESP2 RX16, ESP1 RX33←ESP2 TX17, common GND, 9600 baud.
...
- **PIN-gated admin area** (added): the portal's `/admin` (unlock with the NVS PIN `apin`, default **`1234`**, cleared every `portalStop`) exposes — **SD logs** (list/`/view` inline tail 16 KB/`/download` whole file), **Format SD** = *delete all files* (SD.h has no true format on core 2.0.17; confirm-gated, refused while a summary is mid-read), **edit owner number** (`PHONE_NUMBER`), and **config forms**: **ThingSpeak keys** (`TSKEY`), **per-column targets/preset/mode/name** (`SET`/`MODE`/`NAME`), and **thresholds** (`THRESH`). Path params are sanitized to a root `*.CSV` basename (no traversal).
- **Config edits reuse `handleSms`:** each config form builds an SMS-format command **server-side** and enqueues it (`portalCfgQ`, 6-slot ring); the **core-1 loop** pops one when `uiMode==UI_DATA` and replays it through `handleSms()` with `smsMute` set (no ACK-SMS per edit). No parser duplication, no cross-core config write. **New `THRESH,<start>,<stop>,<gap>` SMS command** makes thresholds SMS-configurable too (previously LCD-only). TSKEY keys are shown as set/unset in the UI, never echoed.
- **Portal robustness invariants:** all core0→core1 String hand-offs (`pendingOwner`/`pendingAdminPin`/`portalCfgQ`) are published **under `netMux`** (the drain releases `netMux` **before** calling `handleSms`, which re-takes it); `saveTsKey`/`saveWifi` also copy the creds under `netMux` before the NVS write. Routes are registered **once** (a `routesRegistered` static — `WebServer::stop()` doesn't free handlers, so re-registering per session would leak). The nearby-SSID list is **scanned once** at `portalStart` into `portalScanOpts` (a `/rescan` route refreshes it) — never per `GET /`, which in AP_STA would bump the phone off the AP. All env/user strings in portal HTML (SSID options, column name, owner) go through `htmlEscape()` (a hostile nearby SSID on the pre-PIN root page must not inject script).
- **Manual reboot (no reset button):** **Settings → "Reboot ESP1"** (`SET_RESET`, NO/YES double-confirm) and the SoftAP admin **"Reboot ESP1"** button both set `rebootPending`; the **core-1 loop** does `logFlush(true)` then `ESP.restart()` (never `ESP.restart()` straight from the core-0 portal). Config persists in NVS; ESP2 keeps executing any in-flight run and re-syncs on ESP1's return.
- **SD cross-core invariant:** all SD access — `logFlush`, the SMS summary parser (core 1), and the portal handlers (core 0) — is serialized by a FreeRTOS **`sdMux`**. `logFlush` uses a timed take and buffers on miss (never blocks the core-1 loop / WDT); summaries are **deferred while `portalActive`** and format waits for `summaryStage==SUM_IDLE`. **Owner number + admin PIN are staged to core 1** (`pendingOwner`/`pendingAdminPin` + persist-flags) exactly like the WiFi creds — never written from core 0, so `senderIsOwner`/`sendSMS` stay lock-free. `PHONE_NUMBER` + `apin` persist in NVS.
...
- **No automatic faults / e-stop during Testing:** while `sysState == TEST_MODE`, `raiseFault()` returns immediately (records `TEST_SUPPRESSED|…` to the log, but no UI-preempt, no SMS alert, no `enterEmergencyStop`), `heartbeatTick()` skips the ESP2/Nano silence recovery (no mid-test power-cycle), and ESP2's `pcfHealth()` no-ops in `testMode`. Testing is manual + dead-man + hard-cap guarded, so the operator is the safety. The **manual MODE+BACK** e-stop still works (it calls `enterEmergencyStop` directly). Single-relay hard cap raised **10 s → 30 s** (`TEST_HARD_CAP_MS`); combos already 30 s.
