...
- **Portal robustness invariants:** all core0→core1 String hand-offs (`pendingOwner`/`pendingAdminPin`/`portalCfgQ`) are published **under `netMux`** (the drain releases `netMux` **before** calling `handleSms`, which re-takes it). Routes are registered **once** (a `routesRegistered` static — `WebServer::stop()` doesn't free handlers, so re-registering per session would leak). The nearby-SSID list is **scanned once** at `portalStart` into `portalScanOpts` (a `/rescan` route refreshes it) — never per `GET /`, which in AP_STA would bump the phone off the AP.
