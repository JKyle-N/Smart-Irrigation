...
## CHANGED: battery/INA226 protective actions removed (operator request)
- Battery voltage no longer triggers **any** action: no `BATTERY_CRITICAL` emergency-stop, no `if (batteryCritical) return` run-block, no low-battery fertigation disable, no `BATTERY_LOW`/`BATTERY_CRITICAL` SMS, no low-battery pump-exercise skip, and **INA226 is excluded from the device-loss auto-reboot** (`deviceHealthTick` skips `DEV_INA`).
- **INA226 monitoring is retained** — `batteryCritical`/`batteryLow` are now display/log-only flags; battV/battI/battP still feed the LCD, `NET`, ThingSpeak, the PWR log, and daily energy Wh. (Trade-off: no brown-out / deep-discharge protection.)
